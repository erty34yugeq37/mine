...
export class MMBClient {
    ...
    constructor() {
        ...
        const dbg = () => typeof window !== 'undefined' && !!localStorage.getItem('ghoul:debug');
        this.socket.on('connect', () => { if (dbg()) console.debug('[Ghoul] socket connected', this.socket.id); });
        this.socket.on('connect_error', (e: any) => { if (dbg()) console.debug('[Ghoul] socket connect_error', e?.message || e); });
        this.socket.on('disconnect', (reason: string) => { if (dbg()) console.debug('[Ghoul] socket disconnected', reason); });
    }
    ...
    startAttack(p: StartAttackPayload) {
        const dbg = () => typeof window !== 'undefined' && !!localStorage.getItem('ghoul:debug');
        if (dbg()) console.debug('[Ghoul] emitting startAttack', p);
